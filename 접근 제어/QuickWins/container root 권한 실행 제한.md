...
- **적용 시 고려사항:** `readOnlyRootFilesystem: true`와 non-root 실행을 적용하면 컨테이너 내부의 임의 쓰기는 제한되지만, 애플리케이션이 정상 동작을 위해 사용하는 `/tmp`, `/var/cache`, `/var/run`, 로그 디렉터리 같은 경로까지 쓰기가 차단되어 가용성 문제가 발생할 수 있다. 따라서 전체 root 파일시스템을 쓰기 가능하게 되돌리지 말고, 실제로 쓰기가 필요한 경로만 `emptyDir` 또는 PVC로 분리해 명시적으로 write 권한을 제공해야 한다.
